def par(a, b, c, d):                                                # Функция определения параллелограмма
    ...
    nnn = (min(povtor)**2 - (min(nepovtor)/2)**2)**0.5              # отрезок от вершины до пересечения диагоналей
    mmm = (((max(nepovtor)-nnn)**2 + (min(nepovtor)/2)**2))**0.5    # высчитываем длину стороны фигуры

    # И если эта длина совпадает с какой-либо длиной уже высчитанной в начале, значит перпендикуляры пересекаются
    # под прямым углом и это уже не параллелограмм.
    if povtor.count(mmm) > 0:
